...
export function addWidgetToSection(type, sectionId) {
  const section = findNode(state.elements, sectionId)
...
/* Move an existing node to a list, optionally before a target. */
export function moveNodeTo(id, targetId = null, position = 'inside') {
  if (!id || id === targetId) return false
  const source = findParentList(state.elements, id)
  if (!source) return false
  const target = targetId ? findNode(state.elements, targetId) : null

  // Never allow a section to be dropped into itself or one of its descendants.
  ...
  const targetParent = findParentList(state.elements, target.id)
  if (!targetParent) {
    state.elements.push(node)
    selectNode(node.id)
    return true
  ...
  let targetIndex = targetParent.list.indexOf(target)
  if (position === 'after') targetIndex += 1
  targetParent.list.splice(targetIndex, 0, node)
  selectNode(node.id)
  return true
}
...
export function dropWidget(type, targetId = null, position = 'inside') {
  ...
  if (target && position === 'inside' && target.type === 'section') {
    target.children.push(node)
  } else if (target) {
