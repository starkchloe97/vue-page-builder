...
import { state, LABELS, nodeStyle, selectNode, deleteNode, duplicateNode, moveNode, moveNodeTo } from '../state.js'
...
function onDrop(event) {
  ...
  if (widgetType) {
    if (props.node.type === 'section') {
      const { dropWidget } = requireState()
      dropWidget(widgetType, props.node.id, 'inside')
    } else {
      const { dropWidget } = requireState()
      dropWidget(widgetType, props.node.id, 'after')
    }
  } else if (nodeId) {
    const rect = event.currentTarget.getBoundingClientRect()
    const position = props.node.type === 'section' && event.clientY > rect.top + rect.height * 0.25 && event.clientY < rect.bottom - rect.height * 0.25
      ? 'inside'
...
// Kept as a tiny indirection so the recursive component has no circular import issues.
function requireState() {
  // dropWidget is statically imported below through the module binding.
  return { dropWidget }
}
</script>
...
<template>
  <div
    ...
    @dragover="onDragOver"
    @dragleave="state.dragOverId === node.id && (state.dragOverId = null)"
    @drop="onDrop"
    @click.stop="onSelect"
  >
  ...
  <component v-else-if="node.type === 'heading'" :is="node.content.tag" class="el-node type-heading" :class="{ selected: isSelected, 'drag-over': isDragOver }" :style="style" draggable="true" @dragstart="onDragStart" @dragover="onDragOver" @dragleave="state.dragOverId === node.id && (state.dragOverId = null)" @drop="onDrop" @click.stop="onSelect">
    {{ node.content.text }}
    <div class="el-toolbar" v-if="isSelected">
      <span class="el-label">{{ LABELS[node.type] }}</span><button title="Move up" @click.stop="moveNode(node.id, -1)">↑</button><button title="Move down" @click.stop="moveNode(node.id, 1)">↓</button><button title="Duplicate" @click.stop="duplicateNode(node.id)">⧉</button><button title="Delete" @click.stop="deleteNode(node.id)">✕</button>
    </div>
  </component>
  ...
  <a v-else-if="node.type === 'button'" class="el-node type-button" :class="{ selected: isSelected, 'drag-over': isDragOver }" :href="node.content.link" @click.prevent.stop="onSelect" :style="style" draggable="true" @dragstart="onDragStart" @dragover="onDragOver" @dragleave="state.dragOverId === node.id && (state.dragOverId = null)" @drop="onDrop">
    {{ node.content.text }}
    <div class="el-toolbar" v-if="isSelected"><span class="el-label">{{ LABELS[node.type] }}</span><button title="Move up" @click.stop="moveNode(node.id, -1)">↑</button><button title="Move down" @click.stop="moveNode(node.id, 1)">↓</button><button title="Duplicate" @click.stop="duplicateNode(node.id)">⧉</button><button title="Delete" @click.stop="deleteNode(node.id)">✕</button></div>
  </a>
...
<script>
// Static import for the recursive SFC's drop handler.
import { dropWidget } from '../state.js'
</script>
